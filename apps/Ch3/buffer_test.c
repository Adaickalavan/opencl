...
#define PROGRAM_FILE "./apps/Ch3/buffer_test.cl"
...
int main() {
   ...
   /* Data and buffers */
   float full_matrix[80], zero_matrix[80], temp_matrix[80];
   ...
   matrix_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | 
      CL_MEM_COPY_HOST_PTR, sizeof(zero_matrix), zero_matrix, &err);
   if(err < 0) {
   ...
   err = clEnqueueReadBuffer(queue, matrix_buffer, CL_TRUE, 0,
      sizeof(temp_matrix), temp_matrix, 0, NULL, NULL);
   if(err < 0) {
      perror("Couldn't read the buffer object");
      exit(1);   
   }
   ...
   for(i=0; i<8; i++) {
      for(j=0; j<10; j++) {
         printf("%6.1f", temp_matrix[j+i*10]);
      }
      printf("\n");
   }
   printf("Zero matrix\n");
